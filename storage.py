...
class ObjectStorage:
    ...
    def __init__(self, sqlite_db_path: str, table_name: str):
        ...
        self.table_name = ObjectStorage._scrub(table_name)
    ...
    @staticmethod
    def _scrub(table_name: str):
        ''' Allows only alphanumerics and underscore in the storage table name and tags. '''

        if ''.join( char for char in table_name if (char.isalnum() or char == '_' )) != table_name:
            raise Exception('Table name or tag must only contain alphanumerics and underscore.')
        else:
            return table_name
    ...
    def create_table(self) -> None:
        ...
        self.sqlite_cursor.execute('''CREATE TABLE IF NOT EXISTS {0} (
            ...
            tag TEXT UNIQUE,
            object BLOB
        )'''.format(ObjectStorage._scrub(self.table_name)))
    ...
    def store(self, tag: str, object_: Any) -> None:
        ...
        try:
            self.sqlite_cursor.execute('''INSERT INTO {0} (tag, object) VALUES (?, ?)'''.format(
                ObjectStorage._scrub(self.table_name)),
                (ObjectStorage._scrub(tag), pickle.dumps(object_, pickle.HIGHEST_PROTOCOL)))
    ...
    def get(self, tag: str) -> Any:
        ...
        self.sqlite_cursor.execute('SELECT id, tag, object FROM {0} WHERE tag=?'.format(
            ObjectStorage._scrub(self.table_name)), (tag, ))
